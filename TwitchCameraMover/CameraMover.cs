...
namespace TwitchCameraMover
{
    class CameraMover : MonoBehaviour
    {
        ...
        public CameraMoverConfig Config;
        ...
        float minCameraPos = -1000f;
        float maxCameraPos = 1000f;
        ...
        public void slideCamera(float x, float y, float z, float duration, string moveType)
        {
            x = Mathf.Clamp(x, minCameraPos, maxCameraPos);
            y = Mathf.Clamp(y, minCameraPos, maxCameraPos);
            z = Mathf.Clamp(z, minCameraPos, maxCameraPos);
            duration = Mathf.Clamp(duration, 0.001f, maxDuration);
            lerpSpeed = duration;
            SlideAction.SlideMotionType slideMotionType = SlideAction.SlideMotionType.Linear;
            if (!string.IsNullOrEmpty(moveType))
            {
                switch (moveType)
                {
                    ...
                    case "ease":
                    case "easeinout":
                        slideMotionType = SlideAction.SlideMotionType.EaseInOut;
                        break;
                    case "b":
                    case "bez":
                    case "bezier":
                        slideMotionType = SlideAction.SlideMotionType.Bezier;
                        break;
                }
            }
            ...
            var slideAction = new SlideAction(startPos, new Vector3(x, z, y), duration, slideMotionType);
            slideActionQueue.Enqueue(slideAction);
        }
    }
}
